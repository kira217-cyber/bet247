...
const AddGameCategory = () => {
  ...
  useEffect(() => {
    axios
      .get("https://apigames.oracleapi.net/api/providers", {
        headers: {
          "x-api-key":
  ...
  useEffect(() => {
    ...
    } else if (editingCategory && !form.mainImage) {
      setMainImagePreview(`${import.meta.env.VITE_API_URL}${editingCategory.mainImage}`);
    } else {
      setMainImagePreview("");
    }
    ...
    } else if (editingCategory && !form.iconImage) {
      setIconImagePreview(`${import.meta.env.VITE_API_URL}${editingCategory.iconImage}`);
    } else {
      setIconImagePreview("");
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCategory) {
        await axios.put(`${import.meta.env.VITE_API_URL}/api/categories/${editingCategory._id}`, formData);
        ...
        <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              accept="image/*"
              required={!editingCategory}
              onChange={(e) => setForm({ ...form, iconImage: e.target.files[0] })}
              className="w-full px-5 py-3 border-2 border-yellow-300 rounded-xl file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-yellow-500 file:text-white hover:file:bg-yellow-600"
            />
            {iconImagePreview && (
              <img
          ...
          <div className="md:col-span-2">
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-4 rounded-xl text-lg transition transform hover:scale-105 disabled:opacity-70 disabled:cursor-not-allowed"
            >
              {loading
                ? "Saving..."
                : editingCategory
            ...
            <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2 flex gap-4 justify-center mt-6">
                <button
                  type="submit"
                  disabled={loading}
                  className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold px-12 py-4 rounded-xl text-lg transition transform hover:scale-105 disabled:opacity-70"
                >
                ...
                <button
                  type="button"
                  onClick={closeModal}
                  className="bg-gray-500 hover:bg-gray-600 text-white font-bold px-12 py-4 rounded-xl text-lg transition"
                >
                  Cancel
                </button>
              </div>
