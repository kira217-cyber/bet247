...
const PlayGame = () => {
  ...
  const { loginUser, fetchUser, userBalance } = useContext(AuthContext);
  ...
  useEffect(() => {
    ...
    const launchGame = async () => {
      try {
        setLoading(true);
        setError("");
        // এখানে money হিসেবে userBalance ব্যবহার করো (পুরো ব্যালেন্স দিয়ে খেলা)
        const money = userBalance || 0;
        if (money <= 0) {
          throw new Error("Insufficient balance to play the game.");
        }
        const res = await axios.post(`${API_URL}/playgame`, {
          gameID: gameId,
        ...
        if (res.data.success) {
          ...
          // AuthContext এ balance রিফ্রেশ করো
    ...
    launchGame();
  }, [gameId, loginUser, navigate, API_URL, fetchUser, userBalance]);
